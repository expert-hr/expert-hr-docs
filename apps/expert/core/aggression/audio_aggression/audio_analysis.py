...
class AudioAggression:
    ...
    def __init__(
        ...
        if isinstance(audio, Tensor):
            ...
            raise TypeError(f"Type '{type(audio)}' is innapropriate for the record.")
    ...
    def _get_average(self) -> None:
        if self.stamps:
            ...
            for num, stamp in enumerate(self.stamps):
                part = self.audio[0][int(stamp[0] * self.sr) : int(stamp[1] * self.sr)]
    ...
    def _get_temp(self) -> None:
        ...
        for elem in self.features:
            part = self.audio[0][
                int(elem * self.sr) : int((elem + self.duration) * self.sr)
            ].numpy()
