...
from apps.expert.core.aggression.video_aggression.anglenet import (
    AngleNet,
    classify_rotation,
)
from apps.expert.core.aggression.video_aggression.face_mesh import (
    HEAD_INDEXES,
    LOW_PART,
    UPP_PART,
    FaceMesh,
...
class VideoAggression:
    ...
    def __init__(
        ...
        self.features = pd.read_json(features_path)
                ...
                mesh_landmarks.append(
                    {
                        ...
                        "upp_part": np.array([face_mesh[idx] for idx in LOW_PART]),
                        "low_part": np.array([face_mesh[idx] for idx in UPP_PART]),
                    }
                )
    ...
    def get_report(self) -> Tuple[List, List, str]:
        ...
        data["upp_coef"] = data["upp_part"].apply(lambda x: np.subtract(mean, x).mean())

        quantile_25, quantile_75 = np.quantile(data["upp_coef"], 0.25), np.quantile(
            data["upp_coef"], 0.75
        )
        quantile_15, quantile_85 = np.quantile(data["upp_coef"], 0.15), np.quantile(
            data["upp_coef"], 0.85
        )
        ...
        data["low_coef"] = data["low_part"].apply(lambda x: np.subtract(mean, x).mean())

        quantile_25, quantile_75 = np.quantile(data["low_coef"], 0.25), np.quantile(
            data["low_coef"], 0.75
        )
        quantile_15, quantile_85 = np.quantile(data["low_coef"], 0.15), np.quantile(
            data["low_coef"], 0.85
        )
        ...
        return (self.div_vid_agg, self.full_vid_agg)
