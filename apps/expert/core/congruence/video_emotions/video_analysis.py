...
from apps.expert.core.functional_tools import get_model_weights_url
...
def get_video_emotions(
    ...
    for row in range(video_length + duration):
        if len(features[features["time_sec"] == row]):
            features_idx = features[features["time_sec"] == row].index[0].item()
            current_frame = cv2.cvtColor(
                video[features.loc[features_idx, "frame_index"]], cv2.COLOR_BGR2RGB
            )
            ...
            lim_emotions = softmax(
                torch.Tensor([[emotions[0], emotions[5], emotions[4]]])
            )[0]
            ...
            if not lim_emotions.isnan().any().item():
                ...
                emo_report.append(
                    {
                        "time_sec": features.loc[features_idx, "time_sec"],
