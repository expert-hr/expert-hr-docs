...
from apps.expert.core.congruence.audio_emotions.audio_model import AudioModel
...
class AudioAnalysis:
    ...
    def __init__(
        ...
        if isinstance(video_path, (str, PathLike)):
            ...
            self.audio = AudioReader(video_path, sample_rate=self.sr, mono=True)[:]
        else:
            raise TypeError
        ...
        if self.stamps:
            for stamp in self.stamps:
                ...
                fragment = self.audio[0][
                    int(stamp[0] * self.sr) : int(stamp[1] * self.sr)
                ]
                self.chunks = self._chunkizer(self.duration, fragment.numpy(), self.sr)

                for num, chunk in enumerate(self.chunks):
                    parts_predict = []
                    self.chunk_parts = self._chunkizer(3, chunk, self.sr)
                    self.chunk_parts = [torch.Tensor(i) for i in self.chunk_parts]
                    self.test = []
                    for i in range(len(self.chunk_parts)):
                        ...
                        self.test.append(
                            torch.tensor(mfcc, dtype=torch.float).to(self._device)
                        )
